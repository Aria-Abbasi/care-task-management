...
function TaskEditDialog({ task, onClose, onSave, locale }: { task: TaskTemplate; onClose: () => void; onSave: (values: Partial<TaskTemplate>) => Promise<void>; locale: string }) {
  ...
  const [requiresNote, setRequiresNote] = useState(task.requires_note)
  const [requiresPhoto, setRequiresPhoto] = useState(task.requires_photo)
  const [error, setError] = useState('')
  const dialogRef = useRef<HTMLFormElement>(null)
  useDialogFocus(dialogRef, onClose)
  const submit = async (event: FormEvent) => { event.preventDefault(); setError(''); try { await onSave({ title, instructions, expected_outcome: expectedOutcome, safety_notes: safetyNotes, requires_note: requiresNote, requires_photo: requiresPhoto }) } catch (requestError) { setError(requestError instanceof Error ? requestError.message : fa ? 'به‌روزرسانی وظیفه ممکن نشد.' : 'Task could not be updated.') } }
  return <div className="modal-layer"><button className="modal-backdrop" onClick={onClose} aria-label={fa ? 'بستن ویرایشگر وظیفه' : 'Close task editor'} tabIndex={-1} /><form ref={dialogRef} className="modal task-form" role="dialog" aria-modal="true" aria-labelledby="edit-task-title" tabIndex={-1} onSubmit={submit}><button type="button" className="modal-close" onClick={onClose} aria-label={fa ? 'بستن' : 'Close'}><X /></button><h2 id="edit-task-title">{fa ? 'ویرایش تعریف وظیفه' : 'Edit task definition'}</h2><label>{fa ? 'نام وظیفه' : 'Task name'}<input required value={title} onChange={(event) => setTitle(event.target.value)} /></label><label>{fa ? 'دستورهای مراقبت' : 'Care instructions'}<textarea value={instructions} onChange={(event) => setInstructions(event.target.value)} /></label><label>{fa ? 'نتیجه مورد انتظار' : 'Expected outcome'}<textarea value={expectedOutcome} onChange={(event) => setExpectedOutcome(event.target.value)} /></label><label>{fa ? 'یادداشت‌های ایمنی' : 'Safety notes'}<textarea value={safetyNotes} onChange={(event) => setSafetyNotes(event.target.value)} /></label><fieldset className="completion-policy compact"><legend>{fa ? 'یادداشت هنگام ثبت انجام' : 'Completion note policy'}</legend><div><label className={!requiresNote ? 'selected' : ''}><input type="radio" name="edit-note-policy" checked={!requiresNote} onChange={() => setRequiresNote(false)} /><span><strong>{fa ? 'اختیاری' : 'Optional'}</strong><small>{fa ? 'انجام عادی بدون توضیح قابل ثبت است.' : 'Routine completion can be saved without a note.'}</small></span></label><label className={requiresNote ? 'selected' : ''}><input type="radio" name="edit-note-policy" checked={requiresNote} onChange={() => setRequiresNote(true)} /><span><strong>{fa ? 'الزامی' : 'Required'}</strong><small>{fa ? 'هر ثبت انجام به توضیح نیاز دارد.' : 'Every completion needs a note.'}</small></span></label></div></fieldset><label className="checkbox completion-photo-policy"><input type="checkbox" checked={requiresPhoto} onChange={(event) => setRequiresPhoto(event.target.checked)} /><span>{fa ? 'عکس انجام الزامی باشد' : 'Require a completion photo'}</span></label>{error && <div className="login-error" role="alert"><AlertCircle />{error}</div>}<div className="modal-actions"><button type="button" className="secondary-button" onClick={onClose}>{fa ? 'انصراف' : 'Cancel'}</button><button className="primary-button">{fa ? 'ذخیره وظیفه' : 'Save task'}</button></div></form></div>
}
