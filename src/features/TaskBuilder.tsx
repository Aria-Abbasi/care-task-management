...
export function TaskBuilder({ patient, token, existingTitles, onClose, onCreate, locale = 'en' }: {
  patient: Patient
  token: string
  ...
  const [frequency, setFrequency] = useState<BuilderFrequency>('DAILY')
  ...
  return <Dialog onClose={onClose} locale={locale}>
    ...
    <form className="task-form builder-form" onSubmit={submit}>
      ...
      {step === 1 && <section className="builder-panel" aria-labelledby="builder-schedule-heading">
        ...
        <div className="schedule-callout"><Clock3 /><span><strong>{fa ? 'پیش‌نمایش زمان‌بندی' : 'Schedule preview'}</strong><small>{scheduleSummary(schedule)}</small></span></div>
      </section>}
      ...
      {step === 2 && <section className="builder-panel" aria-labelledby="builder-care-heading">
        ...
        <div className="form-row"><label className="checkbox"><input type="checkbox" checked={requiresNote} onChange={(event) => setRequiresNote(event.target.checked)} /><span>{fa ? 'یادداشت انجام الزامی باشد' : 'Require completion note'}</span></label><label className="checkbox"><input type="checkbox" checked={requiresPhoto} onChange={(event) => setRequiresPhoto(event.target.checked)} /><span>{fa ? 'عکس انجام الزامی باشد' : 'Require completion photo'}</span></label></div>
