...
export default function UnifiedSchedule({ session, patient, selectedDate, locale, onDate, onRecordOccurrence, onAdd }: Props) {
  ...
  const copy = fa ? { eyebrow: 'تقویم مراقبت', title: 'برنامه مراقبت', action: 'افزودن فعالیت', day: 'روز', week: 'هفته', month: 'ماه', loading: 'در حال بارگذاری تقویم مراقبت', retry: 'تلاش دوباره', noCare: 'برای این بازه فعالیتی برنامه‌ریزی نشده است', noCareDetail: 'یک وظیفه بسازید یا روز دیگری را انتخاب کنید.', coverage: 'پوشش شیفت', availability: 'دسترسی مراقب', today: 'امروز', record: 'ثبت نتیجه ایمن', details: 'جزئیات فعالیت', assigned: 'مسئول', schedule: 'زمان‌بندی‌شده', safety: 'راهنمای ایمنی', close: 'بستن جزئیات', count: 'فعالیت' } : { eyebrow: 'CARE CALENDAR', title: 'Care schedule', action: 'Add task', day: 'Day', week: 'Week', month: 'Month', loading: 'Loading care calendar', retry: 'Try again', noCare: 'No care is scheduled for this period', noCareDetail: 'Create a task or choose another day.', coverage: 'Shift coverage', availability: 'Caregiver availability', today: 'Today', record: 'Record safe outcome', details: 'Activity details', assigned: 'Assigned to', schedule: 'Scheduled', safety: 'Safety guidance', close: 'Close details', count: 'care activities' }
  const load = () => {
  ...
  return <>
    ...
    <section className="unified-calendar" aria-label={copy.title}>
      ...
      {!loading && !error && mode === 'week' && <WeekGrid selected={base} data={data} fa={fa} onDate={onDate} onSelect={setSelected} />}
      {!loading && !error && mode === 'month' && <MonthGrid selected={base} data={data} fa={fa} onDay={goDay} onSelect={setSelected} />}
    </section>
...
function DayAgenda({ date, items, shifts, fa, copy, onSelect }: { date: Date; items: TaskOccurrence[]; shifts: CalendarData['shifts']; fa: boolean; copy: Record<string, string>; onSelect: (item: TaskOccurrence) => void }) {
...
function MonthGrid({ selected, data, fa, onDay, onSelect }: { selected: Date; data: CalendarData | null; fa: boolean; onDay: (date: Date) => Promise<void>; onSelect: (item: TaskOccurrence) => void }) {
  ...
  return <div className="clean-month"><h2>{new Intl.DateTimeFormat(locale, { month: 'long', year: 'numeric' }).format(selected)}</h2><div className="clean-month-weekdays">{weekdays.map((day, index) => <span key={`${day}-${index}`}>{day}</span>)}</div><div className="clean-month-grid">{cells.map((day) => { const items = data?.occurrences.filter((item) => sameDay(new Date(item.effective_scheduled_at), day)) || []; return <section className={`${day.getMonth() === selected.getMonth() ? '' : 'outside'} ${sameDay(day, selected) ? 'active' : ''}`} key={key(day)}><button className="month-day-button" aria-label={new Intl.DateTimeFormat(locale, { dateStyle: 'full' }).format(day)} onClick={() => onDay(day)}>{new Intl.DateTimeFormat(locale, { day: 'numeric' }).format(day)}</button><div>{items.slice(0, 3).map((item) => <button className={`month-task-chip ${item.status.toLowerCase()}`} key={item.id} onClick={() => onSelect(item)}>{item.task_detail.title}</button>)}{items.length > 3 && <button className="month-more" onClick={() => onDay(day)}>+{items.length - 3}</button>}</div></section> })}</div></div>
}
