...
export default function TimelineView({ session, patient, dashboard, locale }: { session: Session; patient: Patient; dashboard: DashboardResponse; locale: string }) {
  ...
  const [events, setEvents] = useState<TimelineEvent[]>(() => baseEvents(dashboard, t))
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setEvents([
        ...baseEvents(dashboard, t),
        ...vitals.map((item) => ({ id: `vital-${item.id}`, kind: 'vital' as const, at: item.recorded_at, title: `${t.recorded}: ${item.type.replace('_', ' ')}`, detail: `${item.value}${item.secondary_value ? `/${item.secondary_value}` : ''} ${item.unit} · ${item.recorded_by_name || 'Haven'}` })),
        ...
        ...audits.map((item) => ({ id: `audit-${item.id}`, kind: 'audit' as const, at: item.created_at, title: t.action, detail: item.summary })),
        ...messages.map((item) => ({ id: `message-${item.id}`, kind: 'message' as const, at: item.created_at, title: t.sent, detail: `${item.sender_name}: ${item.body || (item.voice_note ? 'Voice note' : 'Secure attachment')}`, status: item.urgent ? 'urgent' : undefined })),
      ].sort((a, b) => new Date(b.at).getTime() - new Date(a.at).getTime()))
    } catch {
      setEvents(baseEvents(dashboard, t)); setError(t.error)
    } finally { setLoading(false) }
  }, [dashboard, patient.id, session.token, t])
  useEffect(() => { load() }, [load])
  ...
  return <>
    ...
    <section className="main-card timeline-view">
      ...
      {!loading && <div className="timeline-list">{filtered.map((event) => <article className={`timeline-event ${event.kind}`} key={event.id}><span className="timeline-event-icon">{icon(event.kind)}</span><div><small>{new Intl.DateTimeFormat(fa ? 'fa-IR-u-ca-persian' : undefined, { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(event.at))}</small><strong>{event.title}</strong><p>{event.detail}</p></div>{event.status && <b>{event.status}</b>}</article>)}{!filtered.length && <div className="empty-care compact"><History /><strong>{t.none}</strong></div>}</div>}
    </section>
  </>
}

function baseEvents(dashboard: DashboardResponse, text: { occurrence: string; dose: string }) {
  return [
    ...dashboard.occurrences.map((item) => ({ id: `task-${item.id}`, kind: 'task' as const, at: item.completed_at || item.effective_scheduled_at, title: `${text.occurrence}: ${item.task_detail.title}`, detail: `${item.status.toLowerCase()} · ${item.task_detail.assigned_to_name || item.task_detail.patient_name}`, status: item.status.toLowerCase() })),
