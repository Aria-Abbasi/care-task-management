...
function App() {
  ...
  return (
    <div className="app-shell">
      ...
      {selectedTask && <TaskModal task={selectedTask} patient={patient} canRecord={user.role !== 'FAMILY'} onClose={() => setSelectedTask(null)} onSave={recordTaskOutcome} onDelay={delayTask} onSkip={skipTask} onPhoto={(occurrenceId) => downloadTaskCompletionPhoto(session.token, occurrenceId)} locale={locale} />}
...
export function TaskModal({ task, patient, canRecord = true, onClose, onSave, onDelay, onSkip, onPhoto, locale = 'en' }: { task: CareTask; patient: Patient; canRecord?: boolean; onClose: () => void; onSave: (task: CareTask, payload: Record<string, unknown>, correction?: boolean) => Promise<void>; onDelay?: (task: CareTask, delayedUntil: string, reason: string) => Promise<void>; onSkip?: (task: CareTask, outcome: 'UNABLE' | 'REFUSED', note: string) => void | Promise<void>; onPhoto?: (occurrenceId: number) => void | Promise<void>; locale?: string }) {
  const fa = locale === 'fa'
