...
function App() {
  ...
  useEffect(() => {
    if (session && !canAccessView(session.user.role, view)) navigate('today')
  ...
  const visibleNavigation = localizedNavigation.filter((item) => canAccessView(user.role, item.id))
  const shellCopy = copy(locale).shell
    ...
    <div className="app-shell">
      ...
      <aside className={`sidebar ${mobileMenu ? 'sidebar-open' : ''}`}>
        ...
        <button className="patient-mini" onClick={() => setPatientMenu(!patientMenu)} aria-expanded={patientMenu} aria-haspopup="listbox">
          ...
          <div><strong>{patient.full_name}</strong><span>{shellCopy.assigned(patients.length)}</span></div>
          <ChevronDown size={17} />
        </button>
      ...
      <main id="main-content">
        <header className="topbar">
          <div className="topbar-left">
            ...
            <div><span className="mobile-page-title">{activeLabel}</span><span className="breadcrumb">{shellCopy.careOf(patient.first_name)} <ChevronRight size={13} /> {activeLabel}</span></div>
          </div>
          <div className="topbar-actions">
            <button className={`sync-status ${syncing ? 'syncing' : !online ? 'offline' : pendingSync ? 'pending' : 'synced'}`} onClick={() => pendingSync ? setView('safety') : syncNow()} title={online ? shellCopy.pendingTitle(pendingSync) : shellCopy.offlineTitle} disabled={syncing} aria-live="polite">
              {syncing ? <RefreshCw className="spinning" /> : online ? <Cloud /> : <CloudOff />}
              <span>{syncing ? shellCopy.syncing : !online ? shellCopy.offline : pendingSync ? shellCopy.pending(pendingSync) : shellCopy.synced}</span>
            </button>
      ...
      {selectedTask && <TaskModal task={selectedTask} patient={patient} canRecord={true} canDelayOrSkip={user.role !== 'FAMILY'} canCorrect={user.role !== 'FAMILY'} onClose={() => setSelectedTask(null)} onSave={recordTaskOutcome} onDelay={delayTask} onSkip={skipTask} onPhoto={(occurrenceId) => downloadTaskCompletionPhoto(session.token, occurrenceId)} locale={locale} />}
...
export function Dashboard({
  ...
  const canManageTasks = user.role !== 'FAMILY'
  const isPersian = locale === 'fa'
  ...
  return (
    <>
      <section className="welcome-row">
        ...
        {canManageTasks && <button className="primary-button" onClick={onAdd}><Plus size={19} /> {copy.addTask}</button>}
      </section>
...
function Medications({ session, patient, medications, doses, onDose, onPrn, notify, onRefresh, locale }: { session: Session; patient: Patient; medications: Medication[]; doses: DoseLog[]; onDose: (dose: DoseLog) => void; onPrn: (medication: Medication) => void; notify: (message: string) => void; onRefresh: () => Promise<void>; locale: string }) {
  const fa = locale === 'fa'
  const medicationCopy = copy(locale).medication
  ...
  const doseStatus = (status: DoseLog['status']) => ({
    SCHEDULED: medicationCopy.scheduled,
    GIVEN: medicationCopy.given,
    MISSED: medicationCopy.missed,
    REFUSED: medicationCopy.refused,
    HELD: medicationCopy.held,
  }[status])
...
export function TaskModal({ task, patient, canRecord = true, canDelayOrSkip = true, canCorrect = true, onClose, onSave, onDelay, onSkip, onPhoto, locale = 'en' }: { task: CareTask; patient: Patient; canRecord?: boolean; canDelayOrSkip?: boolean; canCorrect?: boolean; onClose: () => void; onSave: (task: CareTask, payload: Record<string, unknown>, correction?: boolean) => Promise<void>; onDelay?: (task: CareTask, delayedUntil: string, reason: string) => Promise<void>; onSkip?: (task: CareTask, outcome: 'UNABLE' | 'REFUSED', note: string) => void | Promise<void>; onPhoto?: (occurrenceId: number) => void | Promise<void>; locale?: string }) {
  const fa = locale === 'fa'
